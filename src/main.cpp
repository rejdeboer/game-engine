    ...
    while (quit == false) {
        ...
        for (int row = 0; row < n_tiles_y; row++) {
            for (int col = 0; col < n_tiles_x; col++) {
                ...
                TileChunk *chunk = get_tile_chunk(
                    world->tile_map, chunk_pos.chunk_x, chunk_pos.chunk_y);

                if (chunk &&
                    chunk->tiles[chunk_pos.tile_y * tile_map->chunk_dim +
                                 chunk_pos.tile_x]) {
                    draw_rect(gRenderer, tile_x, tile_y,
                              tile_map->tile_side_in_pixels,
        ...
        draw_rect(gRenderer, player_left_x, player_bottom_y,
                  ...
                  -PLAYER_HEIGHT * tile_map->meters_to_pixels, 1.0f, 0.0f,
                  0.0f);
